...
#define shiftLatchPin D5
#define shiftClockPin D3
#define shiftDataPin  D4

/* Chip control */
#define chipEnable   D0
#define outputEnable D6
#define WE           D7
    ...
    case WRITE: {
      ...
      while (ij <= end_address) {
        if (Serial.available() >= 2) {
          uint8_t dataA = Serial.read(); // Сначала принимаем младший байт
          uint8_t dataB = Serial.read(); // Затем старший байт
            if (dataA != 0xFF || dataB != 0xFF) //Запись должна выполняться только тогда, когда и A не равно 0xFF, ИЛИ B не равно 0xFF.
              {  
    ...
    default: {
      if (Serial.available() > 0) {
        ...
        switch (incomingByte) {
          ...
          case 'z': {
            // Ждем прихода 4 байт с количеством слов от ПК
            ...
            // Делаем поправку: end_address = кол-во_слов - 1
            if (wordsCount > 0) {
            ...
            chip = MB32; // Включаем произвольный режим
            break;
          }
        }
      }
